...
public class Convertor {
    ...
    public void init() {
        ...
        literalList.add(" Z.DOTORI_CL_CD, A.dotori_cl as dotori_cl, B.dotori_cl AS DOTORI_CL");
    ...
    public String convert(String literal, List<String> filterList) {
    	...
    	for(String filterWord :filterList) {	// 변경대상문자와 변경결과 키워드를 루핑하면서, 변경대상문자열을 비교한다.
    		...
    		// 치환대상 문자열내에 
    		if(!asisWord.equals(tobeWord) && literal.contains(asisWord) && !literal.contains(tobeWord)) { // 변경대상 문자열에, 변경대상문자(ASIS)는 포함, 변경결과문자(TOBE)는 제외한다. 
